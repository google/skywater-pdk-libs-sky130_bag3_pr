...
class MOSTechSkywater130(MOSTech):
    ...
    def __init__(
        self, tech_info: TechInfo, lch: int, arr_options: Mapping[str, Any]
    ) -> None:
    ...
    def well_w_edge(self) -> int:
        ...
        return (
            -(self.sd_pitch - self.lch) // 2 + self.od_po_extx + nwell_imp + imp_od_encx
        )
    ...
    def get_track_specs(self, conn_layer: int, top_layer: int) -> List[TrackSpec]:
        ...
        return [
            TrackSpec(
                layer=lay,
                direction=Orient2D.y,
                width=vm_w,
                space=num_sd * sd_pitch - vm_w,
                offset=(num_sd * sd_pitch) // 2,
            )
            for lay, vm_w, num_sd in grid_info
            if conn_layer <= lay <= top_layer
        ]
    ...
    def get_mos_row_info(
        ...
        conn_layer: int,
        specs: MOSRowSpecs,
        bot_mos_type: MOSType,
        top_mos_type: MOSType,
        ...
        md_yl, md_yh, _ = self._get_conn_params(
            self.get_conn_info(0, False), od_yl, od_yh
        )
    ...
    def get_ext_width_info(
        self,
        bot_row_ext_info: RowExtInfo,
        top_row_ext_info: RowExtInfo,
        ignore_vm_sp_le: bool = False,
    ) -> ExtWidthInfo:
    ...
    def get_extension_regions(
        self, bot_info: RowExtInfo, top_info: RowExtInfo, height: int
    ...
    def get_mos_conn_info(
        ...
        md_y = (md_yl, md_yh)
        self._draw_ds_conn(
            builder, d0_info, od_y, md_y, num_vc, d_xc, num_d, conn_pitch
        )
        self._draw_ds_conn(
            builder, d0_info, od_y, md_y, num_vc, s_xc, num_s, conn_pitch
        )
        ...
        if export_mid:
            ...
            self._draw_ds_conn(
                builder, d0_info, od_y, md_y, num_vc, m_xc, num_m, wire_pitch
            )
        else:
            m_info = None
        ...
        be = BlkExtInfo(
            ...
            row_info.threshold,
            False,
            ImmutableList([(fg, row_type)]),
            ImmutableSortedDict(),
        )
        return MOSLayInfo(
            builder.get_info(bbox),
            edge_info,
            edge_info,
            be,
            be,
            g_info=(g_xc, num_g, g_pitch),
            d_info=(d_xc, num_d, conn_pitch),
            s_info=(s_xc, num_s, conn_pitch),
            m_info=m_info,
    ...
    def _draw_g_conn(
        ...
        builder.add_rect_arr(
            po_lp,
            BBox(po_xl_even, po_y_gate[0], po_xh_even, po_y_gate[1]),
            nx=(fg - (fg // 2)),
            spx=conn_pitch,
        )
        builder.add_rect_arr(
            po_lp,
            BBox(po_xl_odd, po_y_gate[0], po_xh_odd, po_y_gate[1]),
            nx=(fg // 2),
            spx=conn_pitch,
        )
        ...
        npc_box = BBox(
            g_xc - npc_w2, po_yc_gate - npc_h2, g_xc + npc_w2, po_yc_gate + npc_h2
        )
        ...
        if sep_g:
            ...
            builder.add_rect_arr(
                po_lp,
                BBox(mp_xl, po_y_gate[0], mp_xl + po_w_min, po_y_gate[1]),
                nx=num_g,
                spx=conn_pitch,
            )
        else:
    ...
    def _draw_ds_conn(
        ...
        builder.add_rect_arr(
            ("licon1", "drawing"), vc_box, nx=nx, spx=spx, ny=num_vc, spy=vc_p
        )
        builder.add_rect_arr(("li1", "drawing"), md_box, nx=nx, spx=spx)

    def get_mos_abut_info(
        self, row_info: MOSRowInfo, edgel: MOSEdgeInfo, edger: MOSEdgeInfo
    ) -> LayoutInfo:
        raise ValueError("This method is not supported in this technology.")

    def get_mos_tap_info(
        self, row_info: MOSRowInfo, conn_layer: int, seg: int, options: Param
    ) -> MOSLayInfo:
        assert conn_layer == 0, "currently only work for conn_layer = 0"
        row_type = row_info.row_type
        ...
        if typel == typer:
            ...
            if typel is not row_type:
                if delta_implant > blk_xh:
                    raise ODImplantEnclosureError(
                        "Insufficient space to satisfy "
                        "implant-OD horizontal enclosure."
                    )
                add_base(
                    builder, typel, threshold, imp_y, BBox(0, 0, delta_implant, blk_yh)
                )
                xl = delta_implant
    ...
    def get_mos_ext_info(
        self,
        num_cols: int,
        blk_h: int,
        bot_einfo: RowExtInfo,
        top_einfo: RowExtInfo,
        gr_info: Tuple[int, int],
    ) -> ExtEndLayInfo:
        raise ValueError("Not implemented.")

    def get_mos_ext_gr_info(
        self,
        num_cols: int,
        edge_cols: int,
        blk_h: int,
    ...
    def get_ext_geometries(
        ...
        re_bot: RowExtInfo,
        re_top: RowExtInfo,
        be_bot: ImmutableList[BlkExtInfo],
        be_top: ImmutableList[BlkExtInfo],
        cut_mode: MOSCutMode,
        bot_exty: int,
        top_exty: int,
        dx: int,
        ...
        if bot_exty > 0:
            ...
            w_tot = xcur + w_edge
            _add_blk_ext_edge(
                sd_pitch, builder, be_bot[0], dy, ymid, w_edge, 0, well_w_edge
            )
            _add_blk_ext_edge(
                sd_pitch, builder, be_bot[-1], dy, ymid, w_edge, w_tot, well_w_edge
            )
        if top_exty > 0:
            xcur = dx
            for info in be_top:
            ...
            w_tot = xcur + w_edge
            _add_blk_ext_edge(
                sd_pitch, builder, be_top[0], ymid, ytop, w_edge, 0, well_w_edge
            )
            _add_blk_ext_edge(
                sd_pitch, builder, be_top[-1], ymid, ytop, w_edge, w_tot, well_w_edge
            )
    ...
    def get_mos_end_info(
        self, blk_h: int, num_cols: int, einfo: RowExtInfo
    ) -> ExtEndLayInfo:
    ...
    def get_mos_row_edge_info(
        self, blk_w: int, rinfo: MOSRowInfo, einfo: MOSEdgeInfo
    ) -> LayoutInfo:
        blk_h = rinfo.height
        mos_type: MOSType = einfo["mos_type"]
        return self._edge_info_helper(
            blk_w, blk_h, mos_type, rinfo.threshold, rinfo["imp_y"]
        )
    ...
    def get_mos_corner_info(
        self, blk_w: int, blk_h: int, einfo: MOSEdgeInfo
    ) -> CornerLayInfo:
        lch = self.lch
        sd_pitch = self.sd_pitch
        well_w_edge = self.well_w_edge
        ...
        edgel = ImmutableSortedDict(
            dict(dev_type=DeviceType.MOS, lch=lch, margins=x_margins)
        )
        edgeb = ImmutableSortedDict(
            dict(dev_type=DeviceType.MOS, lch=lch, margins=y_margins)
        )
        return CornerLayInfo(builder.get_info(blk_rect), (0, 0), edgel, edgeb)

    @staticmethod
    def _get_conn_params(
        info: ConnInfo, bot_cl: int, bot_ch: int
    ) -> Tuple[int, int, int]:
    ...
    def _add_mos_active(
        ...
        if is_sub:
            ...
            od_tap_extx = mconf[
                "od_tap_extx"
            ]  # determines the amount to extend material from licon
        else:
            od_lp = ("diff", "drawing")
        ...
        if is_sub:
            ...
            add_base_mos(
                builder, row_info.row_type, row_info.threshold, row_info["imp_y"], bbox
            )

        return od_yl, od_yh

    def _add_po_array(
        self, builder: LayoutInfoBuilder, po_y: Tuple[int, int], start: int, stop: int
    ) -> None:
        mconf = self.mos_config
        po_lp = self.tech_info.config["mos_lay_table"]["PO"]
        lch = self.lch
        ...
        if po_y[1] > po_y[0]:
            builder.add_rect_arr(
                po_lp, BBox(po_x0, po_y[0], po_x0 + lch, po_y[1]), nx=fg, spx=sd_pitch
            )

    def _edge_info_helper(
        self,
        blk_w: int,
        blk_h: int,
        row_type: MOSType,
